...
void offsetKmFromCenter(float lat, float lon, float* dx_km, float* dy_km,
  ...
  *dx_km = static_cast<float>(std::remainder(lon - services::location::lon(), 360.0)) * kKmPerDeg *
           cosf(center_lat_rad);
  *dy_km =
...
int speedLineLengthPx(float gs_knots) {
  if (!std::isfinite(gs_knots) || gs_knots <= 0.0f) {
    return 0;
  }
...
void drawHeadingTriangle(int cx, int cy, float heading_deg, uint16_t color) {
  if (!std::isfinite(heading_deg)) { s_draw->fillCircle(cx, cy, 3, color); return; }
  constexpr float kDegToRad = 0.01745329252f;
  const float rad = heading_deg * kDegToRad;
...
void drawSpeedVector(int cx, int cy, float heading_deg, float track_deg,
  ...
  if (!std::isfinite(heading_deg) || !std::isfinite(track_deg)) return;
