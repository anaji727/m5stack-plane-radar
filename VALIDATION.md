...
- 機種名など空の項目には行を割り当てず、コールサインの直下に高度を表示。
- ラベル全体の高さと画面内への配置も、実際の表示行数で計算。
- Basic用ビルドと配布バイナリの整合性を確認。実機表示は未確認。

## 2026-09-20 近い順一覧と横画面配置

- M5Stackのoffset_rotation=3を確認。rotation=1の実寸は320×240。前版の縦画面という説明と配置座標を訂正。
- 右80pxに距離昇順のコールサイン一覧（最大8機、残数表示、0機表示）と状態表示を追加。
- 設定画面の長押し案内も240pxの高さに収めた。
- Basic用ビルドと配布バイナリの整合性確認を実施。実機での一覧・文字配置は未確認。
...
- アプリサイズ 1,202,517 / 1,310,720 bytes（91.7%）。4 MBパーティション構成内に収まる。
- 生成された `firmware.bin` は 1,208,816 bytes。
